...
def hardnet_loss(X, Y, margin=0.5, anchorSwap=False, random=False, mask=None):
    ...
    dist_neg = dist_mat + 100.0 * torch.eye(
        *dist_mat.size(),
        ...
        device=(
            dist_mat.get_device() if dist_mat.is_cuda else torch.device("cpu")
        )
    )
    ...
    if mask is not None:
        ...
        for i in range(len(mask) - 1):
            mask_filter[mask[i] : mask[i + 1], mask[i] : mask[i + 1]] = 100.0
    ...
    if random:  # Triplet Loss on random negatives
        ...
        cols = torch.randint(
            dist_neg.size(1), size=(dist_neg.size(0),), device=dist_neg.device
        )
        dist_neg = dist_neg[rows, cols]
        loss = torch.clamp(margin + dist_pos - dist_neg, min=0.0)
...
def SSIMLoss(p1, p2):
    return kornia.losses.ssim_loss(
        p1.mean(1, keepdim=True), p2.mean(1, keepdim=True), window_size=7
    )


def sharpness_loss(img):
    sharpness = kornia.filters.laplacian(img, 3).std()
    return torch.clamp(0.1 - sharpness, min=0.0) * 2.0
