    ...
    def detectAndCompute(
        ...
        for scale in scales:
            with torch.no_grad():
                ...
                img = (
                    torch.tensor(img, dtype=torch.float32, device=self.dev)
                    .permute(2, 0, 1)
                    .unsqueeze(0)
                    / 255.0
                )
                ...
                kpts, descs = (
                    kpts[0]["xy"].cpu().numpy().astype(np.int16),
                    descs[0].cpu().numpy(),
                )
        ...
        cv_kps = [
            cv2.KeyPoint(all_kpts[i][0], all_kpts[i][1], 6, 0, all_scores[i])
...
class KeypointSampler(nn.Module):
    ...
    def forward(self, x):
        ...
        idx_cells = self.gridify(
            torch.dstack(
                torch.meshgrid(
                    torch.arange(x.shape[-2], dtype=torch.float32),
        ...
        keypoints = (
            torch.gather(idx_cells, -1, idx.repeat(1, 2, 1, 1).unsqueeze(-1))
            .squeeze(-1)
            .permute(0, 2, 3, 1)
        )
...
class Matcher(nn.Module):
    ...
    def forward(self, x, y, T=1.0):
        Dmat = 2.0 - torch.cdist(x.unsqueeze(0), y.unsqueeze(0)).squeeze(0)
        ...
        seq = torch.arange(
            choice_cols.shape[0],
            dtype=choice_cols.dtype,
            device=choice_cols.device,
        )
        ...
        dmatches = torch.cat(
            (choice_cols[mutual].unsqueeze(-1), seq[mutual].unsqueeze(-1)),
            dim=1,
        )
...
class DEAL(nn.Module):
    ...
    def forward(self, x, NMS=False, threshold=3.0, return_tensors=False, top_k=None):
        ...
        if top_k is not None:
            for b in range(B):
                scores = out["map"][b].squeeze(0)[
                    kpts[b]["xy"][:, 1].long(), kpts[b]["xy"][:, 0].long()
                ]
        ...
        optimize_tps = (
            self.mode == "ts2"
            or self.mode == "end2end-tps"
            or self.mode == "end2end-full"
            or self.mode == "ts-fl"
        )
...
class ProbabilisticDEAL(nn.Module):
    ...
    def __init__(
        ...
        **tps_kwargs
        ...
        # 使用新的概率TPS网络
        self.tps_net = ProbabilisticThinPlateNet(
            in_channels=enc_channels[-1],
            nchannels=enc_channels[0],
            fixed_tps=fixed_tps,
    ...
    def forward(self, x, in_imgs, keypts, Ho, Wo):
        ...
        for b in range(B):
            if keypts[b]["xy"] is not None and len(keypts[b]["xy"]) >= 16:
                ...
                )  # interpolate param tensor @ kp pos
                ...
                I_polargrid = theta.new(
                    N, H, W, 3
                )  # create a new tensor with identity polar grid (normalized by keypoint attributes)
                I_polargrid[..., 0] = 1.0
...
class ProbabilisticThinPlateNet(nn.Module):
    ...
    def _sample_tps_parameters(self, theta_mu, theta_var, num_samples):
        ...
        T_dist = distributions.studentT.StudentT(
            df=2 * alpha, loc=theta_mu, scale=torch.sqrt(theta_var / alpha)
        )
