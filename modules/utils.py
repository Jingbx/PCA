...
def make_batch_sfm(augmentor, difficulty=0.3, train=True):
    ...
    with torch.no_grad():  # we dont require grads in the augmentation
        ...
        p2, H2 = augmentor(
            batch_images, difficulty, TPS=True, prob_deformation=0.7
        )
        ...
        for b in range(augmentor.batch_size):
            Hs.append(
                (
                    H2[b] @ torch.inverse(H1[b]).to(dev),
                    src[b].unsqueeze(0),
                    W[b].unsqueeze(0),
                    A[b].unsqueeze(0),
                )
...
def get_reward(kps1, kps2, H, augmentor, penalty=0.0, px_thr=2):
    with torch.no_grad():
        ...
        rewards[rewards == 0.0] = penalty
    return rewards, reward_sum


def get_dense_rewards(kps1, kps2, H, augmentor, penalty=0.0, px_thr=1.5):
    with torch.no_grad():
        # perspective transform 2 -> 1
        if not augmentor.TPS:
            warped = augmentor.warp_points(torch.inverse(H), kps2)
...
def get_positive_corrs(kps1, kps2, H, augmentor, i=0, px_thr=1.5):
    with torch.no_grad():
        ...
        if not augmentor.TPS:
            ...
            H, src, W, A = H
            undeformed = augmentor.denorm_pts_grid(
                warp_points_tps(augmentor.norm_pts_grid(kps2["xy"]), src, W, A)
            ).view(-1, 2)
            warped = augmentor.warp_points(torch.inverse(H), undeformed)
