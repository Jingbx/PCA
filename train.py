...
def parseArg():
    ...
    parser.add_argument(
        "-m",
        "--mode",
        help="Training mode scheme - two stage (ts) is the default",
        required=False,
        choices=["end2end-backbone", "end2end-tps", "end2end-full", "ts1", "ts2", "ts-fl"],
        default="ts1",
    )
    parser.add_argument(
        "-dpath",
    ...
    parser.add_argument(
        "-log",
        "--logdir",
        help="Output path where results will be saved.",
        required=False,
        default="./log_dir",
    )
    parser.add_argument(
        "-s", "--save", help="Path for saving model", required=False, default="./log_dir"
    )
    parser.add_argument(
        "--dry_run",
        help="Sanity check, overfit the training loop without saving anything",
        action="store_true",
        default=True,
    )
    parser.add_argument("-gpu", "--gpu", help="GPU number", required=False, default=None)
    parser.add_argument(
        "-pre",
        "--pretrained",
        help="Pretrained network path for second stage",
        required=False,
        default="log_dir/model_ts1_10_final.pth",
    )
...
def train(args):
    ...
    if not torch.cuda.is_available():
        # raise RuntimeError(
        #     "Do you really want to train without a GPU? Then comment out this if"
        # )
        dev = torch.device("cpu")
        print("Train model on cpu...")
    else:
        dev = torch.device("cuda")
        print("Train model on cuda...")
    ...
    num_grad_accs = (
        4  # this performs grad accumulation to simulate larger batch size, set to 1 to disable;
    )
    ...
    augmentor = AugmentationPipe(
        ...
        max_num_imgs=(
            3_000 if not args.dry_run else 32
        ),  # Limit number of images in training, original impl is 5000
        num_test_imgs=100,
    ...
    if args.mode == "ts2" or args.mode == "ts-fl":
        ...
        if args.mode == "ts-fl":
            ...
            extractor.net.fusion_layer = nn.Sequential(
                nn.Linear(128, 128),
                nn.ReLU(),
                nn.Linear(128, 128),
                nn.Sigmoid(),
    ...
    fp_penalty = 0.0  # -1e-7 #-0.25
    kp_penalty = -7e-5  # -7e-5 #-0.03
    T = 7.0  # inv of softmax temperature
    ...
    with tqdm.tqdm(total=steps) as pbar:
        for i in range(steps):
            ...
            for b in range(batch_size):
                ...
                if (
                    kpts1[b]["patches"] is None
                    or kpts2[b]["patches"] is None
                    or len(kpts1[b]["xy"]) < 16
                    or len(kpts2[b]["xy"]) < 16
                ):
                    print("skipping batch item...")
                    continue

                idx, patches1, patches2 = get_positive_corrs(
                    kpts1[b], kpts2[b], Hs[b], augmentor, i
                )
                ...
                if len(patches1) >= 16:
                    ...
                    if args.mode == "end2end-backbone" or args.mode == "ts1":
                        ...
                        if args.mode != "end2end-tps":
                            rdesc1 = net.sample_descs(
                                out1["feat"][b],
                                kpts1[b]["xy"][idx[:, 0], :],
                                H=p1.shape[2],
                                W=p1.shape[3],
                            )
                            rdesc2 = net.sample_descs(
                                out2["feat"][b],
                                kpts2[b]["xy"][idx[:, 1], :],
                                H=p2.shape[2],
                                W=p2.shape[3],
                            ...
                            pdesc1 = (
                                l_pdesc1 if pdesc1 is None else torch.vstack((pdesc1, l_pdesc1))
                            )
                            pdesc2 = (
                                l_pdesc2 if pdesc2 is None else torch.vstack((pdesc2, l_pdesc2))
                            )
                            ...
                            pdesc1nr = (
                                l_pdesc1_nrigid
                                if pdesc1nr is None
                                else torch.vstack((pdesc1nr, l_pdesc1_nrigid))
                            )
                            pdesc2nr = (
                                l_pdesc2_nrigid
                                if pdesc2nr is None
                                else torch.vstack((pdesc2nr, l_pdesc2_nrigid))
                            )
                    ...
                    with torch.no_grad():
                        good_matches = torch.argmin(
                            torch.cdist(l_pdesc1, l_pdesc2), dim=1
                        ) == torch.arange(len(l_pdesc1), device=l_pdesc1.device)
                ...
                dense_kp_logprobs = kpts1[b]["logprobs"].view(-1, 1) + kpts2[b]["logprobs"].view(
                    1, -1
                )
                dense_logprobs = dense_kp_logprobs
                dense_rewards, dense_rwd_sum = get_dense_rewards(
                    kpts1[b]["xy"],
                    kpts2[b]["xy"],
                    Hs[b],
                    augmentor,
                ...
                if i > 0.75 * steps and not args.mode == "ts1":  # penalyze wrong matches
                    with torch.no_grad():
                        if len(good_matches) == len(idx):
                            ...
                            dense_rewards[idx[:, 0]] = kp_penalty * 10.0  # 10. 25.
                ...
                current_loss_kp = (
                    kpts1[b]["logprobs"] * torch.full_like(kpts1[b]["logprobs"], kp_penalty * alpha)
                ).mean() + (
                    kpts2[b]["logprobs"] * torch.full_like(kpts2[b]["logprobs"], kp_penalty * alpha)
                ).mean()
                loss_kp = (
                    current_loss_kp if loss_kp is None else torch.hstack((loss_kp, current_loss_kp))
                )
            ...
            pbar.set_description(
                "L: {:.4f} - Det: ({:d}, {:d}), #rwd: {:.0f}/{:d}, #dRwd: {:.1f} #HL: {:.3f} ssimL: {:.3f}".format(
                    loss.item(),
                    det_kpts1,
                    det_kpts2,
                    good_matches.sum(),
                    len(good_matches),
                    dense_correct / batch_size,
                    hard_loss.item() if hard_loss is not None else 0.0,
                    (ssimloss.mean().item() * 2.0 if ssimloss is not None else 0.0),
                )
            )
            pbar.update(1)
            ...
            logger.log_scalars(
                i,
                avg_det=(det_kpts1 + det_kpts2) / 2.0,
                acc=np.array(acc).mean() if len(acc) > 0 else 0.0,
                inliers=good_matches.sum(),
                kp_rewards=dense_correct / batch_size,
                hard_loss=(hard_loss.item() if i > 150 and hard_loss is not None else 0.0),
                ssim_loss=(
                    ssimloss.mean().item() * 2.0 if i > 150 and ssimloss is not None else 0.0
                ),
            )
            ...
            if i % 5000 == 0:
                if not args.dry_run:
                    torch.save(
                        net.state_dict(),
                        args.save + "/model_" + args.mode + "_%06d" % i + ".pth",
                    )
        #     scheduler.step()
    ...
    torch.save(
        net.state_dict(),
        args.save + "/model_" + args.mode + "_" + str(i + 1) + "_final" + ".pth",
    )
